...
file1 = open("circuit.txt","a")
file1.write("\nINPUT A B C D X \n")
file1.write("OUTPUT Z net_e net_f net_g X")
file1.close()
import time
def AssignFaults(a):
...
for elements in f:
    ...
    if not elements.startswith("INPUT"):
        if not elements.startswith("OUTPUT"):
            ...
            if not elements.startswith("FAULT_TYPE"):
                if not elements.startswith("FAULT_AT"):
                    TempGate["GateType"] = templist[2]
                    TempGate["In1Node"] = templist[1]
                    TempGate["In2Node"] = templist[3]
                    TempGate["OutNode"] = templist[0]
                    #print(Gate)
                    #continue
            if elements.startswith("BUF") or elements.startswith("~"):
...
valid = ["1111","1110","1101","1100","1011","1010","1001","1000","0111","0110","0101","0100","0011","0010","0001","0000"]
for UserInput in valid:
...
for elements in Gates:
    if elements["GateType"] == "~" or elements["GateType"] == "BUF":
        if elements["Exec"] == 0:
